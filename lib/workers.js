      ...
      checks.forEach(function (check) {
        _data.read('checks', check, function (err, originalCheckData) {
          if (!err && originalCheckData) {
            // Pass it to the check validator and let that function continue of log errors as needed
...
workers.performCheck = function (originalCheckData) {
  ...
  const _moduleToUser = originalCheckData.protocol === 'http' ? http : https;

  const req = _moduleToUser.request(requestDetails, function (res) {
          ...
          if (!err) {
            _logs.truncate(logId, function (err) {
              if (!err) {
                ...
                console.error('Error truncating logfile');
              }
            });
          } else {
...
// Timer to execute the log-roation once per day
