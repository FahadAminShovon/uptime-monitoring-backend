...
cli.processInput = function (str) {
  ...
  if (str) {
    // Codify the unique strings that identify the unique questions allowed to be
    var uniqueInputs = [
      'man',
      'help',
      'exit',
    ...
    uniqueInputs.some(function (input) {
      if (str.toLowerCase().indexOf(input) > -1) {
        matchFound = true;
        // Emit an event matching the unique input and include th full string given by the user
        e.emit(input.toLowerCase(), str);
        return true;
      }
    });
